...
description: "The complete integration handbook for POST /v2/public/leads — authentication, every field, matching, webhooks and reference data."
---
...
> [`provident-lead-intake-openapi.json`](/api-reference/openapi.json) next to
...
> **Changed 2026-09-19 — read this if you are already integrated.**
> 1. New **`comments[]`** on both `POST` and `PUT`/`PATCH` `/v2/public/leads` (§4.11) —
>    relay a conversation that happened somewhere else onto the lead's own thread, up to
>    five comments per request, **each with its own `date`** so a chat sent an hour later
>    still reads in the order it happened.
> 2. Both responses now carry **`comments: { added, skipped }`** (`null` when you sent
>    none). **`skipped` is customer messages that were not kept** — going over the cap of
>    five does not fail your request, so nothing else will tell you.
> 3. Nothing existing changed. If you do not send `comments`, every request and response
...
```json
{
  ...
  "comments": [
    {
      "body": "Hi, I'm looking for a 2-bed in JVC under 1.3M",
      "date": "2026-09-18T06:10:00.000Z",
      "type": "whatsapp_inbound",
      "chatChannel": "whatsapp"
    },
    {
      "body": "Happy to help — are you buying to live in or to invest?",
      "date": "2026-09-18T06:12:30.000Z",
      "type": "whatsapp_outbound",
      "chatChannel": "whatsapp"
    },
    {
      "body": "To invest. Budget is around 1.3M, cash.",
      "date": "2026-09-18T06:15:00.000Z",
      "type": "whatsapp_inbound",
      "chatChannel": "whatsapp",
      "isPinned": true
    }
...
only changes who receives it if Provident has configured a routing rule that reads it. Sending
one is safe: a value no rule claims changes nothing, and it is a label you can send today and
Provident can act on later, without you changing anything.

**Where a rule does claim the value, it wins.** Provident's routing rules are tried in a set
order, and a lead usually matches the first rule whose conditions fit — but a lead that names a
distribution type goes to the rule that claims that type, ahead of that order. So a lead sent as
`Location distribution` that also carries a campaign is routed by the location rule, not by the
campaign rule that would otherwise have taken it first. Ask which types are claimed before you
rely on this; it is configuration on our side and it changes.
...
check the value if you expect routing to depend on it. Capitalisation is not a typo: `default`,
`Default` and `Default Distribution` are all the same value, on your side and on ours.
...
### 4.11 `comments` — relaying a conversation

For the case the rest of this API has no answer for: the enquiry that produced the lead
happened **somewhere else**, in a chat, and the lead arrives without it. `comments[]`
writes that conversation onto the lead's own thread, where the agent already reads
everything else about it.

Each entry is one comment:

| Field | Type | Required | Meaning |
|---|---|---|---|
| `body` | `string` | yes | The text. Trimmed. Longer than 5000 characters is truncated, not rejected. |
| `date` | `string` (ISO-8601) | no | When it was **said** — see below. Defaults to now. |
| `type` | `string` (≤64) | no | What kind of comment it is, so the CRM can render it as more than a note. Free text today: stored verbatim, matched against nothing. Agree a vocabulary with us and keep it stable. |
| `chatChannel` | `string` (≤64) | no | The conversation it came from — `whatsapp`. Free text, stored verbatim. |
| `isPinned` | `boolean` | no | Pin it to the top of the thread. Default `false`. |

Accepted on **`POST`** and on **`PUT`/`PATCH`**, identically. On an update they are
**added** to what the lead already has; nothing you sent earlier is replaced or removed.

#### `date` is not the time you send the request

This is the whole point of the field. A conversation relayed an hour after it happened is
still displayed, and still sorted, at the time each message was **actually sent** — so it
reads in order beside the agent's own notes rather than arriving as a block stamped
"now". The time it reached us is recorded separately and is never lost.

Two limits: an unparseable date falls back to now, and a date **more than 24 hours in the
future** is ignored the same way — a clock-skewed sender cannot park a message at the top
of a thread forever. A date in the past is always honoured; that is what the field is for.

#### The order is the order you send

**The first entry in the array is the eldest**, and appears at the bottom of the CRM
timeline. Entries you send without a date keep the order you sent them in. Entries that
carry the same timestamp — a transcript accurate only to the minute — also keep it.

#### Maximum 5 per request, and going over does not fail

Anything past the fifth entry is **skipped**, and so is any entry with an empty body. The
request still succeeds, because failing it would throw away a real customer enquiry over a
comment. The count comes back as `comments.skipped` on the response:

```json
"comments": { "added": 5, "skipped": 2 }
```

**Read it.** A sender that ignores it loses customer messages silently. Send a longer
conversation as consecutive `PUT`s, five at a time, oldest batch first.

#### What these are, and are not

- They are **visible** comments — not internal notes, not an audit trail.
- They are **not agent activity**: posting them never restarts the lead's response clock
  and never counts as somebody having worked it.
- `@`-mentions in the text notify nobody.
- They are attributed to your integration, not to a person.

One caveat worth knowing before you build on this: comments are a **placeholder for a
proper chat view**, chosen because it puts the conversation in front of an agent today.
Send `type` and `chatChannel` on every message even though nothing reads them yet — they
are what will let these be lifted into a real chat later without guesswork.
...
| `comments` | `{ added, skipped }` when you sent a `comments[]` array (§4.11), `null` when you did not. **`skipped` is customer messages we did not keep** — send them again. |
...
the status is still `201` but the body is **only these few fields**:
...
```json
{
  ...
  "comments": { "added": 2, "skipped": 0 }
}
```

Any `comments[]` you sent are written to the **surviving** lead — the conversation belongs
on the lead the customer actually has. `comments` is `null` when you sent none.
...
5. **Comments are added, never replaced.** `comments[]` (§4.11) appends up to five
   comments per request to the lead's thread, oldest first. Nothing you sent earlier is
   touched, so a long conversation can be relayed across consecutive calls and still read
   in order.
...
An update carrying **only** `comments` posts no such note — the comments are themselves
what the agent reads, and a note announcing them would sit on top of them on every batch.
...
```json
{
  ...
  "comments": [
    {
      "body": "Following up — is the Marina unit still available?",
      "date": "2026-09-19T05:40:12.000Z",
      "type": "whatsapp_inbound",
      "chatChannel": "whatsapp"
    }
  ]
...
```json
{
  ...
  "updated": true,
  "updatedFields": ["budgetMin", "budgetMax", "closingAreaId", "quizAnswers", "comments"],
...
| `updatedFields` | The lead columns this submission actually changed, by **CRM** name (`closingAreaId` is what `areaOfInterest` fills). `leadPhones` / `leadEmails` appear when a new one was added, `quizAnswers` when answers were written, `metaFacebook` when the Meta block was stored, `comments` when at least one comment was posted. |
...
| `comments` | `{ added, skipped }` when you sent a `comments[]` array (§4.11), `null` when you did not. |
